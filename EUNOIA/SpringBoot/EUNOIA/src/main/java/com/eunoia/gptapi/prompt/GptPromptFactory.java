...
public class GptPromptFactory {
  ...
  public String emotionAnalysis(String content) {
    return """
        ...
        **따뜻하고 진심 어린 시선으로** 이해해 주세요.
        ...
        - **entryClarityScore**
          이 감정글이 이후 메타 분석(사용자의 외적/내면 흐름 분석)에 사용할 만큼 맥락과 감정 흐름이 충분히 담겨 있는지를 0~100 점수로 표현해 주세요.
          (0에 가까울수록 정보가 부족하고, 100에 가까울수록 흐름이 선명해요.)

        - **entryClarityReason**
          위 점수를 그렇게 준 이유를 한문장으로 설명해 주세요.
          (평가/판정이 아니라, “어떤 정보가 선명했는지/부족했는지” 중심으로 적어 주세요.)
  ...
  // meta 전용 프롬프트 (세부조정 필요)
  public String metaAnalysis(String metaInput) {
    return """
        ※ 이 분석은 사용자를 정의하거나 판단하기 위한 것이 아니에요.
          기록에 비친 흐름을 정리해 보여주고,
          사용자가 스스로를 다시 바라볼 수 있도록 돕는 것이 목적이에요.

        당신은 사람을 분석하는 전문가가 아니라,
        기록을 함께 바라보며 흐름을 정리해주는 **조용한 관찰자**예요.

        지금부터 보여드릴 내용은
        한 사용자가 최근 30일 동안 남긴 감정 기록 중,
        의미 있는 흐름을 가진 대표 기록들을 모아둔 것이에요.

        각 기록을 개별적으로 평가하지 말고,
        **기록들 사이에 반복되거나 이어지는 감정의 흐름**에 집중해 주세요.
        ...
        모든 문장은 **해요체**로 작성해 주세요.
        사용자를 지칭할 때는 반드시 **‘당신’**이라는 표현만 사용해 주세요.

        ---
        ✅ 출력 규칙
        - 반드시 아래 스키마를 정확히 포함한 **하나의 JSON 객체만** 출력해 주세요.
        - JSON 이외의 설명/인삿말/코드블록 표시는 절대 포함하지 마세요.
        - 배열 필드는 문자열 배열로 채워 주세요.
        - clarity.clarityScore는 0~100 사이의 정수로 작성해 주세요.

        ---
        ✅ 각 JSON 필드는 아래 설명을 기준으로 작성해 주세요.

        [공통 / 신뢰 레이어]
        - clarity.clarityScore
          : 이 분석이 얼마나 사용자의 흐름을 나타내고 있는지를 0~100 사이 숫자로 표현해 주세요.
        - clarity.clarityReasons
          : 왜 이 정도 선명도로 판단했는지에 대한 이유를 짧은 문장으로 정리해 주세요.
        - clarity.notVisibleYet
          : 기록 수나 내용의 한계로 인해 아직 잘 보이지 않는 영역을 짧은 문장으로 리스트 형식에 맞게 1~3문장정도 간단히 적어 주세요.
        - clarity.nextActions
          : 선명도를 높이기 위해 사용자가 다음에 시도해볼 수 있는 기록 방향을 리스트 형식에 맞게 1~3문장정도 제안해 주세요.

        [외적의 나 (outer)]
        ※ 외적의 나는 ‘기록에 실제로 드러난 모습’만을 다룹니다.
        ※ 행동, 표현, 반복되는 상황 위주로 작성해 주세요.
        ※ 내면의 의도나 욕구를 추정하지 마세요.

        - outer.summary
          : 기록 전반에서 드러난 외적인 감정 흐름과 행동 패턴을 요약해 주세요.
        - outer.keywords
          : 기록에 자주 등장하거나 반복된 감정·상태를 단어로 정리해 주세요.
        - outer.triggers
          : 감정이 촉발된 상황이나 계기로 반복적으로 등장한 요소를 적어 주세요.
        - outer.emotionFlows
          : 감정이 어떻게 시작되어 다른 감정으로 이어지는지의 흐름을 정리해 주세요.
        - outer.copingPatterns
          : 감정에 반응하거나 회복하려는 방식이 어떻게 나타났는지 적어 주세요.
        - outer.strengthSignals
          : 기록에서 드러난 강점이나 안정적인 반응을 적어 주세요.
        - outer.sensitivePoints
          : 감정이 쉽게 흔들리거나 예민해지는 지점을 적어 주세요.

        [내면의 나 (inner)]
        ※ 내면의 나는 ‘기록에 반복적으로 드러난 방향성’을 바탕으로 조심스럽게 해석합니다.
        ※ 행동이나 상황 설명은 쓰지 말고, 그 이면의 욕구·가치·동기에 집중해 주세요.
        ※ 단정하지 말고 가능성이나 경향으로 표현해 주세요.

        - inner.summary
          : 기록 전반에서 드러나는 내면의 방향성과 중심 주제를 요약해 주세요.
        - inner.keywords
          : 내면을 설명하는 핵심 단어를 정리해 주세요.
        - inner.coreValues
          : 기록 속에서 중요하게 여겨지는 가치나 기준을 적어 주세요.
        - inner.needs
          : 반복적으로 드러나는 욕구나 필요를 적어 주세요.
        - inner.innerMotivations
          : 행동의 이면에서 작동하는 내적 동기를 조심스럽게 추정해 주세요.
        - inner.outerInnerGap
          : 외적으로 보이는 모습과 내면의 방향성이 어긋나 보이는 지점을 적어 주세요.
        - inner.gapExplanation
          : 그 엇갈림이 왜 나타났을 수 있는지 흐름 중심으로 설명해 주세요.

        ---
        ✅ 반드시 아래 JSON 스키마를 그대로 따르세요 (키 이름/구조 유지)
        - evidenceNarrative는 서버에서 채우는 영역이에요.
        - 반드시 아래 형태를 유지하고, howChosen은 빈 배열([])로만 출력해 주세요.
